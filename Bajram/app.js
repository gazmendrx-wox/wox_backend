...
app.get('/reviews/:columnName/:value', async (req,res) => {
...
app.post('/reviews/update', async (req, res) => {
  ...
  try {
    ...
    // Use a parameterized query to prevent SQL injection
    const result = await client.query(
      `UPDATE public.reviews SET value='${value}' WHERE id=${id} RETURNING value;`
    );
...
app.post('/reviews/create', async (req, res) => {
  ...
  try {
    ...
    // Use a parameterized query to prevent SQL injection
    const result = await client.query(
      'INSERT INTO reviews (value) VALUES ($1) RETURNING value',
      [value]
    );
